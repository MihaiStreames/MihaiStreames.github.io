...
The PE header has a hard minimum size and `FileAlignment` padding means every section costs at least 512 bytes on disk. Without any size optimizations, the linker produces four sections:
...
`.rdata` is read-only data (string literals, import thunks). `.pdata` is the structured exception handling table the compiler emits for every function on x64, for the Windows stack unwinder. Four sections with minimal actual content still occupy 4 × 512 = 2,048 bytes on disk minimum, plus rounding. Merging all of them into `.text` eliminates the extra section table entries and collapses four alignment boundaries into one:
